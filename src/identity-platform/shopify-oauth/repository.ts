...
import type { ShopifyOAuthConnectionView, ShopifyStoreView } from "./types"
...
function mapConnection(row: Record<string, unknown>): ShopifyOAuthConnectionView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as ShopifyOAuthConnectionView["status"],
...
function mapShopifyStore(row: Record<string, unknown>): ShopifyStoreView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as ShopifyStoreView["status"],
...
export class ShopifyOAuthRepository
  implements ProviderConnectionLifecycleRepository, ProviderAccountDiscoveryRepository
{
  ...
  async resolveProject(input: {
    ...
    const result = await this.db.query<{ id: string; workspace_id: string | null }>({
      name: "shopify-oauth-resolve-project",
      text: `
  ...
  async savePendingState(input: {
    ...
    await this.db.query({
      name: "shopify-oauth-state-insert",
      text: `
        INSERT INTO shopify_oauth_states (
          id, state, organization_id, workspace_id, project_id, user_id, connection_id,
          shop_domain, requested_scopes, redirect_uri, status, expires_at, created_at, updated_at
        ) VALUES (
  ...
  async findPendingStateByValue(state: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "shopify-oauth-state-find",
      text: "SELECT * FROM shopify_oauth_states WHERE state = $1 LIMIT 1",
      values: [state],
    })
  ...
  async consumeStateOnce(stateId: string, consumedAt: string) {
    const result = await this.db.query({
      name: "shopify-oauth-state-consume",
      text: `
  ...
  async upsertConnection(input: {
    ...
    await this.db.query({
      name: "shopify-oauth-connection-upsert",
      text: `
        INSERT INTO shopify_oauth_connections (
          id, organization_id, workspace_id, project_id, data_source_id, shop_domain,
  ...
  async findConnectionById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "shopify-oauth-connection-find",
      text: "SELECT * FROM shopify_oauth_connections WHERE id = $1 AND deleted_at IS NULL LIMIT 1",
      values: [connectionId],
    })
  ...
  async findConnectionShopDomain(connectionId: string): Promise<string | null> {
    const result = await this.db.query<{ shop_domain: string }>({
      name: "shopify-oauth-connection-find-shop-domain",
      text: "SELECT shop_domain FROM shopify_oauth_connections WHERE id = $1 AND deleted_at IS NULL LIMIT 1",
      values: [connectionId],
    })

    return result.rows[0] ? String(result.rows[0].shop_domain) : null
  }

  async findConnectionOwnershipById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "shopify-oauth-connection-find-ownership",
      text: `
  ...
  async findConnectionByProject(organizationId: string, projectId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "shopify-oauth-connection-find-project",
      text: `
        SELECT *
        FROM shopify_oauth_connections
  ...
  async saveEvent(connectionId: string, eventType: string, metadata: Record<string, unknown>) {
    await this.db.query({
      name: "shopify-oauth-event-insert",
      text: `
        INSERT INTO shopify_oauth_events (id, connection_id, event_type, metadata, created_at)
        VALUES ($1, $2, $3, $4, $5)
      `,
      values: [
        randomUUID(),
  ...
  async appendOutboxEvent(input: {
    ...
    await this.db.query({
      name: "shopify-oauth-outbox-insert",
      text: `
        INSERT INTO outbox_events (
          id, event_type, event_version, aggregate_type, aggregate_id,
          occurred_at, metadata, payload, status, attempts, created_at
        ) VALUES ($1,$2,1,'shopify_oauth_connection',$3,$4,$5,$6,'pending',0,$4)
      `,
      values: [
        randomUUID(),
        input.eventType,
  ...
  async replaceAccessibleCustomerAccounts(input: {
    ...
    await this.db.query({
      name: "shopify-store-deactivate-missing",
      text: `
    ...
    await this.db.query({
      name: "shopify-store-clear-selected",
      text: `
        UPDATE shopify_stores
    ...
    await this.db.query({
      name: "shopify-store-set-selected",
      text: `
        UPDATE shopify_stores
  ...
  async findAccessibleCustomerAccount(connectionId: string, customerId: string) {
    ...
    return result.rows[0] ? mapShopifyStore(result.rows[0]) : null
  }

  async deleteConnectionCascade(connectionId: string) {
    await this.db.query({
      name: "shopify-oauth-delete-outbox-events",
      text: `
        DELETE FROM outbox_events
        WHERE aggregate_type = 'shopify_oauth_connection'
          AND aggregate_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "shopify-oauth-delete-audit-logs",
      text: `
        DELETE FROM audit_logs
        WHERE entity_type = 'shopify_oauth_connection'
          AND entity_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "shopify-store-delete",
      text: "DELETE FROM shopify_stores WHERE connection_id = $1",
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "shopify-oauth-delete-states",
      text: "DELETE FROM shopify_oauth_states WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "shopify-oauth-delete-connection",
      text: "DELETE FROM shopify_oauth_connections WHERE id = $1",
      values: [connectionId],
    })
  }
  ...
  async getRawTokenMaterial(connectionId: string) {
    const result = await this.db.query<{
      ...
      name: "shopify-oauth-token-material-find",
      text: `
  ...
  async updateTokenMaterial(input: {
    ...
    await this.db.query({
      name: "shopify-oauth-token-material-update",
      text: `
        UPDATE shopify_oauth_connections
  ...
  async setConnectionLifecycleStatus(input: {
    ...
    await this.db.query({
      name: "shopify-oauth-connection-lifecycle-update",
      text: `
        UPDATE shopify_oauth_connections
  ...
  async listConnectionIdsByWorkspace(
    ...
    const result = await this.db.query<{ id: string }>({
      name: "shopify-oauth-connections-list-by-workspace",
      text: `
        SELECT id
        FROM shopify_oauth_connections
  ...
  async listRecentOutboxEvents(connectionId: string, limit: number) {
    const result = await this.db.query<{
      ...
      name: "shopify-oauth-outbox-recent-list",
      text: `
        SELECT
          id,
