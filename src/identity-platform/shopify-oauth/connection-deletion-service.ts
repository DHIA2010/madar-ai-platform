...
function assertActorCanManageIntegrations(actor: AuthenticatedActor) {
  if (!actor.roles.includes("owner") && !actor.roles.includes("admin")) {
    throw new IdentityError("SHOPIFY_OAUTH_FORBIDDEN", 403, "security", "Permission denied.")
  }
}

export class ShopifyOAuthConnectionDeletionService {
  constructor(private readonly repository: ShopifyOAuthRepository) {}
  ...
  async deleteConnection(actor: AuthenticatedActor, connectionId: string) {
    ...
    await this.repository.withTransaction(async () => {
      ...
      await this.repository.saveEvent(connectionId, "shopify.oauth.connection.deleted", payload)
      await this.repository.appendAuditLog({
        actorUserId: actor.userId,
        organizationId: ownership.organizationId,
        workspaceId: ownership.workspaceId,
        action: "integration.shopify_oauth.deleted",
        entityId: connectionId,
        metadata: payload,
        createdAt: occurredAt,
      })
      await this.repository.appendOutboxEvent({
        eventType: "shopify.oauth.connection.deleted",
