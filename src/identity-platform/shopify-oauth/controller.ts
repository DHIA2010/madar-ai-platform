...
import type { ShopifyOAuthService } from "./service"
import type { ShopifyOAuthStartInput } from "./types"

const CALLBACK_ERROR_REASON_MAP: Record<string, string> = {
  SHOPIFY_OAUTH_STATE_INVALID: "state_invalid",
  SHOPIFY_OAUTH_STATE_EXPIRED: "state_expired",
  SHOPIFY_OAUTH_STATE_ALREADY_CONSUMED: "state_already_consumed",
  SHOPIFY_OAUTH_SHOP_MISMATCH: "shop_mismatch",
  SHOPIFY_OAUTH_HMAC_INVALID: "hmac_invalid",
  SHOPIFY_OAUTH_TOKEN_EXCHANGE_FAILED: "token_exchange_failed",
  SHOPIFY_OAUTH_ACCOUNT_DISCOVERY_FAILED: "account_discovery_failed",
  SHOPIFY_OAUTH_ACCOUNT_DISCOVERY_EMPTY: "account_discovery_empty",
  SHOPIFY_OAUTH_CONFIGURATION_ERROR: "configuration_error",
}
...
export class ShopifyOAuthController {
  constructor(private readonly service: ShopifyOAuthService) {}

  async start(actor: AuthenticatedActor, input: ShopifyOAuthStartInput) {
  ...
  async callback(_request: IncomingMessage, query: URLSearchParams) {
    ...
    try {
      const completed = await this.service.completeAuthorization(query)
      return {
        status: 302,
        headers: {
