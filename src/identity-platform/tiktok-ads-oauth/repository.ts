...
import type { TikTokAdsAdvertiserAccountView, TikTokAdsOAuthConnectionView } from "./types"
...
function mapConnection(row: Record<string, unknown>): TikTokAdsOAuthConnectionView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as TikTokAdsOAuthConnectionView["status"],
...
function mapTikTokAdsAccount(row: Record<string, unknown>): TikTokAdsAdvertiserAccountView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as TikTokAdsAdvertiserAccountView["status"],
...
export class TikTokAdsOAuthRepository
  implements ProviderConnectionLifecycleRepository, ProviderAccountDiscoveryRepository
{
  ...
  async resolveProject(input: {
    ...
    const result = await this.db.query<{ id: string; workspace_id: string | null }>({
      name: "tiktok-ads-oauth-resolve-project",
      text: `
  ...
  async savePendingState(input: {
    ...
    await this.db.query({
      name: "tiktok-ads-oauth-state-insert",
      text: `
        INSERT INTO tiktok_ads_oauth_states (
  ...
  async findPendingStateByValue(state: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "tiktok-ads-oauth-state-find",
      text: "SELECT * FROM tiktok_ads_oauth_states WHERE state = $1 LIMIT 1",
      values: [state],
    })
  ...
  async consumeStateOnce(stateId: string, consumedAt: string) {
    const result = await this.db.query({
      name: "tiktok-ads-oauth-state-consume",
      text: `
        UPDATE tiktok_ads_oauth_states
  ...
  async upsertConnection(input: {
    ...
    await this.db.query({
      name: "tiktok-ads-oauth-connection-upsert",
      text: `
  ...
  async findConnectionById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "tiktok-ads-oauth-connection-find",
      text: "SELECT * FROM tiktok_ads_oauth_connections WHERE id = $1 AND deleted_at IS NULL LIMIT 1",
      values: [connectionId],
    })
  ...
  async findConnectionOwnershipById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "tiktok-ads-oauth-connection-find-ownership",
      text: `
        SELECT id, organization_id, workspace_id
        FROM tiktok_ads_oauth_connections
        WHERE id = $1
  ...
  async findConnectionByProject(organizationId: string, projectId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "tiktok-ads-oauth-connection-find-project",
      text: `
        SELECT *
        FROM tiktok_ads_oauth_connections
        WHERE organization_id = $1
          AND project_id = $2
          AND provider = 'tiktok_ads'
          AND deleted_at IS NULL
        LIMIT 1
      `,
  ...
  async saveEvent(connectionId: string, eventType: string, metadata: Record<string, unknown>) {
    await this.db.query({
      name: "tiktok-ads-oauth-event-insert",
      text: `
        INSERT INTO tiktok_ads_oauth_events (id, connection_id, event_type, metadata, created_at)
        VALUES ($1, $2, $3, $4, $5)
      `,
      values: [
        randomUUID(),
  ...
  async appendOutboxEvent(input: {
    ...
    await this.db.query({
      name: "tiktok-ads-oauth-outbox-insert",
      text: `
        INSERT INTO outbox_events (
          id, event_type, event_version, aggregate_type, aggregate_id,
          occurred_at, metadata, payload, status, attempts, created_at
        ) VALUES ($1,$2,1,'tiktok_ads_oauth_connection',$3,$4,$5,$6,'pending',0,$4)
      `,
      values: [
        randomUUID(),
        input.eventType,
  ...
  async replaceAccessibleCustomerAccounts(input: {
    ...
    const existingSelected = await this.db.query<{ account_id: string }>({
      name: "tiktok-ads-account-selected",
      text: `
          SELECT account_id
    ...
    await this.db.query({
      name: "tiktok-ads-account-deactivate-missing",
      text: `
        UPDATE tiktok_ads_accounts
        SET status = 'inactive',
            is_selected = false,
            updated_at = now()
        WHERE connection_id = $1
    ...
    for (const account of normalizedAccounts) {
      await this.db.query({
        name: "tiktok-ads-account-upsert",
        text: `
          INSERT INTO tiktok_ads_accounts (
    ...
    await this.db.query({
      name: "tiktok-ads-account-clear-selected",
      text: `
        UPDATE tiktok_ads_accounts
    ...
    await this.db.query({
      name: "tiktok-ads-account-set-selected",
      text: `
        UPDATE tiktok_ads_accounts
  ...
  async listAccessibleCustomerAccounts(connectionId: string) {
    ...
    return result.rows.map(mapTikTokAdsAccount)
  }

  async selectCustomerAccount(connectionId: string, customerId: string) {
    return this.db.withTransaction(async () => {
      await this.db.query({
        name: "tiktok-ads-account-select-clear",
        text: `
          UPDATE tiktok_ads_accounts
          SET is_selected = false,
              updated_at = now()
      ...
      const result = await this.db.query({
        name: "tiktok-ads-account-select-set",
        text: `
          UPDATE tiktok_ads_accounts
          SET is_selected = true,
              updated_at = now()
  ...
  async findAccessibleCustomerAccount(connectionId: string, customerId: string) {
    ...
    return result.rows[0] ? mapTikTokAdsAccount(result.rows[0]) : null
  }

  async deleteConnectionCascade(connectionId: string) {
    await this.db.query({
      name: "tiktok-ads-oauth-delete-outbox-events",
      text: `
        DELETE FROM outbox_events
        WHERE aggregate_type = 'tiktok_ads_oauth_connection'
          AND aggregate_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "tiktok-ads-oauth-delete-audit-logs",
      text: `
        DELETE FROM audit_logs
        WHERE entity_type = 'tiktok_ads_oauth_connection'
          AND entity_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "tiktok-ads-account-delete",
      text: "DELETE FROM tiktok_ads_accounts WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "tiktok-ads-sync-delete-records",
      text: "DELETE FROM tiktok_ads_records WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "tiktok-ads-sync-delete-runs",
      text: "DELETE FROM tiktok_ads_sync_runs WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "tiktok-ads-oauth-delete-events",
      text: "DELETE FROM tiktok_ads_oauth_events WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "tiktok-ads-oauth-delete-states",
      text: "DELETE FROM tiktok_ads_oauth_states WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "tiktok-ads-oauth-delete-connection",
      text: "DELETE FROM tiktok_ads_oauth_connections WHERE id = $1",
      values: [connectionId],
    })
  }
  ...
  async getRawTokenMaterial(connectionId: string) {
    const result = await this.db.query<{
      ...
      name: "tiktok-ads-oauth-token-material-find",
      text: `
  ...
  async updateTokenMaterial(input: {
    ...
    await this.db.query({
      name: "tiktok-ads-oauth-token-material-update",
      text: `
        UPDATE tiktok_ads_oauth_connections
  ...
  async setConnectionLifecycleStatus(input: {
    ...
    await this.db.query({
      name: "tiktok-ads-oauth-connection-lifecycle-update",
      text: `
        UPDATE tiktok_ads_oauth_connections
  ...
  async listConnectionIdsByWorkspace(
    ...
    const result = await this.db.query<{ id: string }>({
      name: "tiktok-ads-oauth-connections-list-by-workspace",
      text: `
        SELECT id
        FROM tiktok_ads_oauth_connections
  ...
  async listRecentOutboxEvents(connectionId: string, limit: number) {
    const result = await this.db.query<{
      ...
      name: "tiktok-ads-oauth-outbox-recent-list",
      text: `
        SELECT
          id,
