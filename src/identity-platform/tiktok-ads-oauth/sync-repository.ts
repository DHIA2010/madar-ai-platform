...
// Real advertiser accounts can have thousands of campaigns/ad groups/ads/insight-days in one
// sync -- one INSERT per row meant one network round-trip per row, which is what made a large
// sync time out in production for other connectors before this batching was added (see
// zid-oauth/sync-repository.ts). 500 keeps each statement's parameter count (7 per row) and
// size well clear of Postgres's own limits.
...
export class TikTokAdsSyncRepository {
  ...
  async createOrLoadSyncRun(input: {
    ...
    const inserted = await this.db.query<Record<string, unknown>>(
      `
      insert into tiktok_ads_sync_runs (
  ...
  async findSyncRunById(syncRunId: string) {
    const result = await this.db.query<Record<string, unknown>>(
      "select * from tiktok_ads_sync_runs where id = $1 limit 1",
      [syncRunId]
    )
  ...
  async upsertRecords(input: {
    connectionId: string
    customerId: string
    records: TikTokAdsSyncRecordInput[]
  }): Promise<number> {
    let written = 0
