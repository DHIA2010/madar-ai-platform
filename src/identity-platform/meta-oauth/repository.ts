...
import type { MetaAdsAccountView, MetaOAuthConnectionView } from "./types"
...
function mapConnection(row: Record<string, unknown>): MetaOAuthConnectionView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as MetaOAuthConnectionView["status"],
...
function mapMetaAccount(row: Record<string, unknown>): MetaAdsAccountView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as MetaAdsAccountView["status"],
...
export class MetaOAuthRepository
  implements ProviderConnectionLifecycleRepository, ProviderAccountDiscoveryRepository
{
  ...
  async resolveProject(input: {
    ...
    const result = await this.db.query<{ id: string; workspace_id: string | null }>({
      name: "meta-oauth-resolve-project",
      text: `
  ...
  async findPendingStateByValue(state: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "meta-oauth-state-find",
      text: "SELECT * FROM meta_oauth_states WHERE state = $1 LIMIT 1",
      values: [state],
    })
  ...
  async upsertConnection(input: {
    ...
    await this.db.query({
      name: "meta-oauth-connection-upsert",
      text: `
  ...
  async findConnectionById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "meta-oauth-connection-find",
      text: "SELECT * FROM meta_oauth_connections WHERE id = $1 AND deleted_at IS NULL LIMIT 1",
      values: [connectionId],
    })
  ...
  async findConnectionOwnershipById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "meta-oauth-connection-find-ownership",
      text: `
  ...
  async findConnectionByProject(organizationId: string, projectId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "meta-oauth-connection-find-project",
      text: `
        SELECT *
        FROM meta_oauth_connections
  ...
  async saveEvent(connectionId: string, eventType: string, metadata: Record<string, unknown>) {
    await this.db.query({
      name: "meta-oauth-event-insert",
      text: `
        INSERT INTO meta_oauth_events (id, connection_id, event_type, metadata, created_at)
        VALUES ($1, $2, $3, $4, $5)
      `,
      values: [
        randomUUID(),
  ...
  async appendOutboxEvent(input: {
    ...
    await this.db.query({
      name: "meta-oauth-outbox-insert",
      text: `
        INSERT INTO outbox_events (
          id, event_type, event_version, aggregate_type, aggregate_id,
          occurred_at, metadata, payload, status, attempts, created_at
        ) VALUES ($1,$2,1,'meta_oauth_connection',$3,$4,$5,$6,'pending',0,$4)
      `,
      values: [
        randomUUID(),
        input.eventType,
  ...
  async replaceAccessibleCustomerAccounts(input: {
    ...
    const existingSelected = await this.db.query<{ account_id: string }>({
      name: "meta-ads-account-selected",
      text: `
          SELECT account_id
    ...
    await this.db.query({
      name: "meta-ads-account-deactivate-missing",
      text: `
    ...
    await this.db.query({
      name: "meta-ads-account-clear-selected",
      text: `
        UPDATE meta_ads_accounts
    ...
    await this.db.query({
      name: "meta-ads-account-set-selected",
      text: `
        UPDATE meta_ads_accounts
  ...
  async selectCustomerAccount(connectionId: string, customerId: string) {
    return this.db.withTransaction(async () => {
      ...
      const result = await this.db.query({
        name: "meta-ads-account-select-set",
        text: `
          UPDATE meta_ads_accounts
          SET is_selected = true,
              updated_at = now()
  ...
  async findAccessibleCustomerAccount(connectionId: string, customerId: string) {
    ...
    return result.rows[0] ? mapMetaAccount(result.rows[0]) : null
  }

  async deleteConnectionCascade(connectionId: string) {
    await this.db.query({
      name: "meta-oauth-delete-outbox-events",
      text: `
        DELETE FROM outbox_events
        WHERE aggregate_type = 'meta_oauth_connection'
          AND aggregate_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "meta-oauth-delete-audit-logs",
      text: `
        DELETE FROM audit_logs
        WHERE entity_type = 'meta_oauth_connection'
          AND entity_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "meta-ads-delete-accounts",
      text: "DELETE FROM meta_ads_accounts WHERE connection_id = $1",
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "meta-oauth-delete-states",
      text: "DELETE FROM meta_oauth_states WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "meta-oauth-delete-connection",
      text: "DELETE FROM meta_oauth_connections WHERE id = $1",
      values: [connectionId],
    })
  }
  ...
  async getRawTokenMaterial(connectionId: string) {
    const result = await this.db.query<{
      ...
      name: "meta-oauth-token-material-find",
      text: `
  ...
  async updateTokenMaterial(input: {
    ...
    await this.db.query({
      name: "meta-oauth-token-material-update",
      text: `
  ...
  async setConnectionLifecycleStatus(input: {
    ...
    await this.db.query({
      name: "meta-oauth-connection-lifecycle-update",
      text: `
        UPDATE meta_oauth_connections
  ...
  async listConnectionIdsByWorkspace(
    ...
    const result = await this.db.query<{ id: string }>({
      name: "meta-oauth-connections-list-by-workspace",
      text: `
        SELECT id
        FROM meta_oauth_connections
  ...
  async listRecentOutboxEvents(connectionId: string, limit: number) {
    const result = await this.db.query<{
      ...
      name: "meta-oauth-outbox-recent-list",
      text: `
        SELECT
          id,
