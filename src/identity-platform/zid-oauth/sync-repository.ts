...
// production for the other connectors before this batching was added. Batching multiple rows
// into a single multi-row INSERT cuts that to one round-trip per batch. 500 keeps each
// statement's parameter count (7 per row) and size well clear of Postgres's own limits.
...
export interface ZidSyncRecordInput {
...
export class ZidSyncRepository {
  ...
  async findSyncRunById(syncRunId: string) {
    const result = await this.db.query<Record<string, unknown>>(
      "select * from zid_sync_runs where id = $1 limit 1",
      [syncRunId]
    )
