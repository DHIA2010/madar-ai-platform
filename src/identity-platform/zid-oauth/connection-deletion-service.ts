...
function assertActorCanManageIntegrations(actor: AuthenticatedActor) {
  if (!actor.roles.includes("owner") && !actor.roles.includes("admin")) {
    throw new IdentityError("ZID_OAUTH_FORBIDDEN", 403, "security", "Permission denied.")
  }
}

export class ZidOAuthConnectionDeletionService {
  constructor(private readonly repository: ZidOAuthRepository) {}
    ...
    await this.repository.withTransaction(async () => {
      ...
      if (!ownership) {
        throw new IdentityError(
          "ZID_OAUTH_CONNECTION_NOT_FOUND",
          404,
          "business",
      ...
      await this.repository.saveEvent(connectionId, "zid.oauth.connection.deleted", payload)
      await this.repository.appendAuditLog({
        actorUserId: actor.userId,
        organizationId: ownership.organizationId,
        workspaceId: ownership.workspaceId,
        action: "integration.zid_oauth.deleted",
        entityId: connectionId,
        metadata: payload,
        createdAt: occurredAt,
      })
      await this.repository.appendOutboxEvent({
        eventType: "zid.oauth.connection.deleted",
