...
import type { SallaOAuthConnectionView, SallaStoreView } from "./types"
...
function mapConnection(row: Record<string, unknown>): SallaOAuthConnectionView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as SallaOAuthConnectionView["status"],
...
function mapSallaStore(row: Record<string, unknown>): SallaStoreView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as SallaStoreView["status"],
...
export class SallaOAuthRepository
  implements ProviderConnectionLifecycleRepository, ProviderAccountDiscoveryRepository
{
  ...
  async resolveProject(input: {
    ...
    const result = await this.db.query<{ id: string; workspace_id: string | null }>({
      name: "salla-oauth-resolve-project",
      text: `
  ...
  async savePendingState(input: {
    ...
    await this.db.query({
      name: "salla-oauth-state-insert",
      text: `
        INSERT INTO salla_oauth_states (
  ...
  async findPendingStateByValue(state: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "salla-oauth-state-find",
      text: "SELECT * FROM salla_oauth_states WHERE state = $1 LIMIT 1",
      values: [state],
    })
  ...
  async consumeStateOnce(stateId: string, consumedAt: string) {
    const result = await this.db.query({
      name: "salla-oauth-state-consume",
      text: `
        UPDATE salla_oauth_states
  ...
  async upsertConnection(input: {
    ...
    await this.db.query({
      name: "salla-oauth-connection-upsert",
      text: `
  ...
  async findConnectionById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "salla-oauth-connection-find",
      text: "SELECT * FROM salla_oauth_connections WHERE id = $1 AND deleted_at IS NULL LIMIT 1",
      values: [connectionId],
    })
  ...
  async findConnectionOwnershipById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "salla-oauth-connection-find-ownership",
      text: `
  ...
  async saveEvent(connectionId: string, eventType: string, metadata: Record<string, unknown>) {
    await this.db.query({
      name: "salla-oauth-event-insert",
      text: `
        INSERT INTO salla_oauth_events (id, connection_id, event_type, metadata, created_at)
        VALUES ($1, $2, $3, $4, $5)
      `,
      values: [
        randomUUID(),
  ...
  async appendOutboxEvent(input: {
    ...
    await this.db.query({
      name: "salla-oauth-outbox-insert",
      text: `
        INSERT INTO outbox_events (
          id, event_type, event_version, aggregate_type, aggregate_id,
          occurred_at, metadata, payload, status, attempts, created_at
        ) VALUES ($1,$2,1,'salla_oauth_connection',$3,$4,$5,$6,'pending',0,$4)
      `,
      values: [
        randomUUID(),
        input.eventType,
  ...
  async replaceAccessibleCustomerAccounts(input: {
    ...
    await this.db.query({
      name: "salla-store-deactivate-missing",
      text: `
        UPDATE salla_stores
        SET status = 'inactive',
            is_selected = false,
    ...
    await this.db.query({
      name: "salla-store-clear-selected",
      text: `
        UPDATE salla_stores
  ...
  async findAccessibleCustomerAccount(connectionId: string, customerId: string) {
    ...
    return result.rows[0] ? mapSallaStore(result.rows[0]) : null
  }

  async deleteConnectionCascade(connectionId: string) {
    await this.db.query({
      name: "salla-oauth-delete-outbox-events",
      text: `
        DELETE FROM outbox_events
        WHERE aggregate_type = 'salla_oauth_connection'
          AND aggregate_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "salla-oauth-delete-audit-logs",
      text: `
        DELETE FROM audit_logs
        WHERE entity_type = 'salla_oauth_connection'
          AND entity_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "salla-store-delete",
      text: "DELETE FROM salla_stores WHERE connection_id = $1",
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "salla-oauth-delete-states",
      text: "DELETE FROM salla_oauth_states WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "salla-oauth-delete-connection",
      text: "DELETE FROM salla_oauth_connections WHERE id = $1",
      values: [connectionId],
    })
  }
  ...
  async getRawTokenMaterial(connectionId: string) {
    const result = await this.db.query<{
      ...
      name: "salla-oauth-token-material-find",
      text: `
  ...
  async updateTokenMaterial(input: {
    ...
    await this.db.query({
      name: "salla-oauth-token-material-update",
      text: `
        UPDATE salla_oauth_connections
  ...
  async setConnectionLifecycleStatus(input: {
    ...
    await this.db.query({
      name: "salla-oauth-connection-lifecycle-update",
      text: `
        UPDATE salla_oauth_connections
  ...
  async listConnectionIdsByWorkspace(
    ...
    const result = await this.db.query<{ id: string }>({
      name: "salla-oauth-connections-list-by-workspace",
      text: `
        SELECT id
        FROM salla_oauth_connections
  ...
  async listRecentOutboxEvents(connectionId: string, limit: number) {
    const result = await this.db.query<{
      ...
      name: "salla-oauth-outbox-recent-list",
      text: `
        SELECT
          id,
