...
export interface SallaSyncRecordInput {
...
export class SallaSyncRepository {
  ...
  async findSyncRunById(syncRunId: string) {
    const result = await this.db.query<Record<string, unknown>>(
      "select * from salla_sync_runs where id = $1 limit 1",
      [syncRunId]
    )
