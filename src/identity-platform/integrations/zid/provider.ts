...
import { ZidSyncRepository } from "../../zid-oauth/sync-repository"
import { ZidSyncService } from "../../zid-oauth/sync-service"
import type { ZidOAuthConnectionView } from "../../zid-oauth/types"
...
export class ZidIntegrationProvider {
  readonly providerId = "zid"
  ...
  private requireController() {
    if (!this.controller) {
      throw new IntegrationProviderError(
        "Zid OAuth requires database-backed identity runtime.",
        "ZID_OAUTH_UNAVAILABLE",
        false,
        503
      )
    }
  ...
  private requireRepository() {
    if (!this.repository) {
      throw new IntegrationProviderError(
        "Zid OAuth repository unavailable.",
        "ZID_OAUTH_UNAVAILABLE",
        false,
        503
      )
  ...
  private assertConnectionOwnership(
    connection: ZidOAuthConnectionView | null,
    actor: AuthenticatedActor
  ): asserts connection is ZidOAuthConnectionView {
