...
describe("google oauth service", () => {
  it("starts oauth, exchanges code, and persists encrypted tokens with audit evidence", async () => {
    ...
    const persisted = await database.query<{
      ...
      "select status, project_id, provider_account_email from google_oauth_connections where id = $1",
      [started.connectionId]
    )
    expect(persisted.rows[0]?.status).toBe("connected")
    expect(persisted.rows[0]?.project_id).toBe(PROJECT_ID)
    expect(persisted.rows[0]?.provider_account_email).toBe("ads-user@example.com")

    const tokenResult = await database.query<{ encrypted_refresh_token: string | null }>(
      "select encrypted_refresh_token from google_oauth_connections where id = $1",
      [started.connectionId]
    )
  ...
  it("fails safely on database write failure and invalid encryption configuration", async () => {
    ...
    const service = new GoogleOAuthService(repository)
    ...
    const invalidKeyService = new GoogleOAuthService(new GoogleOAuthRepository(database), {
