...
// Real accounts can have thousands of rows in one sync (GA4 events/conversions especially) --
// one INSERT per row meant one network round-trip per row, which is what made a large sync
// time out in production. Batching multiple rows into a single multi-row INSERT cuts that to
// one round-trip per batch. 500 keeps each statement's parameter count (7 per row) and size
// well clear of Postgres's own limits while still being a large improvement over one-at-a-time.
