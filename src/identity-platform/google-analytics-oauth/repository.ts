...
import type { GoogleAnalyticsOAuthConnectionView, GoogleAnalyticsPropertyView } from "./types"
...
function mapConnection(row: Record<string, unknown>): GoogleAnalyticsOAuthConnectionView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as GoogleAnalyticsOAuthConnectionView["status"],
...
function mapGoogleAnalyticsProperty(row: Record<string, unknown>): GoogleAnalyticsPropertyView {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as GoogleAnalyticsPropertyView["status"],
...
export class GoogleAnalyticsOAuthRepository
  implements ProviderConnectionLifecycleRepository, ProviderAccountDiscoveryRepository
{
  ...
  async resolveProject(input: {
    ...
    const result = await this.db.query<{ id: string; workspace_id: string | null }>({
      name: "google-analytics-oauth-resolve-project",
      text: `
  ...
  async savePendingState(input: {
    ...
    await this.db.query({
      name: "google-analytics-oauth-state-insert",
      text: `
        INSERT INTO google_analytics_oauth_states (
  ...
  async findPendingStateByValue(state: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "google-analytics-oauth-state-find",
      text: "SELECT * FROM google_analytics_oauth_states WHERE state = $1 LIMIT 1",
      values: [state],
    })
  ...
  async consumeStateOnce(stateId: string, consumedAt: string) {
    const result = await this.db.query({
      name: "google-analytics-oauth-state-consume",
      text: `
        UPDATE google_analytics_oauth_states
  ...
  async upsertConnection(input: {
    ...
    await this.db.query({
      name: "google-analytics-oauth-connection-upsert",
      text: `
        INSERT INTO google_analytics_oauth_connections (
  ...
  async findConnectionById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "google-analytics-oauth-connection-find",
      text: "SELECT * FROM google_analytics_oauth_connections WHERE id = $1 AND deleted_at IS NULL LIMIT 1",
      values: [connectionId],
    })
  ...
  async findConnectionOwnershipById(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "google-analytics-oauth-connection-find-ownership",
      text: `
        SELECT id, organization_id, workspace_id
        FROM google_analytics_oauth_connections
        WHERE id = $1
  ...
  async findConnectionByProject(organizationId: string, projectId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "google-analytics-oauth-connection-find-project",
      text: `
        SELECT *
        FROM google_analytics_oauth_connections
  ...
  async saveEvent(connectionId: string, eventType: string, metadata: Record<string, unknown>) {
    await this.db.query({
      name: "google-analytics-oauth-event-insert",
      text: `
        INSERT INTO google_analytics_oauth_events (id, connection_id, event_type, metadata, created_at)
        VALUES ($1, $2, $3, $4, $5)
      `,
      values: [
        randomUUID(),
  ...
  async appendOutboxEvent(input: {
    ...
    await this.db.query({
      name: "google-analytics-oauth-outbox-insert",
      text: `
        INSERT INTO outbox_events (
          id, event_type, event_version, aggregate_type, aggregate_id,
          occurred_at, metadata, payload, status, attempts, created_at
        ) VALUES ($1,$2,1,'google_analytics_oauth_connection',$3,$4,$5,$6,'pending',0,$4)
      `,
      values: [
        randomUUID(),
        input.eventType,
  ...
  async replaceAccessibleCustomerAccounts(input: {
    ...
    const existingSelected = await this.db.query<{ account_id: string }>({
      name: "google-analytics-property-selected",
      text: `
          SELECT account_id
    ...
    await this.db.query({
      name: "google-analytics-property-deactivate-missing",
      text: `
    ...
    for (const account of normalizedAccounts) {
      await this.db.query({
        name: "google-analytics-property-upsert",
        text: `
          INSERT INTO google_analytics_properties (
    ...
    await this.db.query({
      name: "google-analytics-property-clear-selected",
      text: `
        UPDATE google_analytics_properties
    ...
    await this.db.query({
      name: "google-analytics-property-set-selected",
      text: `
        UPDATE google_analytics_properties
  ...
  async listAccessibleCustomerAccounts(connectionId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "google-analytics-property-list",
      text: `
        SELECT *
        FROM google_analytics_properties
        WHERE connection_id = $1
    ...
    return result.rows.map(mapGoogleAnalyticsProperty)
  }

  async selectCustomerAccount(connectionId: string, customerId: string) {
    return this.db.withTransaction(async () => {
      await this.db.query({
        name: "google-analytics-property-select-clear",
        text: `
          UPDATE google_analytics_properties
          SET is_selected = false,
              updated_at = now()
      ...
      const result = await this.db.query({
        name: "google-analytics-property-select-set",
        text: `
          UPDATE google_analytics_properties
          SET is_selected = true,
              updated_at = now()
  ...
  async findAccessibleCustomerAccount(connectionId: string, customerId: string) {
    const result = await this.db.query<Record<string, unknown>>({
      name: "google-analytics-property-find",
      text: `
        SELECT *
        FROM google_analytics_properties
        WHERE connection_id = $1
    ...
    return result.rows[0] ? mapGoogleAnalyticsProperty(result.rows[0]) : null
  }

  async deleteConnectionCascade(connectionId: string) {
    await this.db.query({
      name: "google-analytics-oauth-delete-outbox-events",
      text: `
        DELETE FROM outbox_events
        WHERE aggregate_type = 'google_analytics_oauth_connection'
          AND aggregate_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "google-analytics-oauth-delete-audit-logs",
      text: `
        DELETE FROM audit_logs
        WHERE entity_type = 'google_analytics_oauth_connection'
          AND entity_id = $1
      `,
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "google-analytics-property-delete",
      text: "DELETE FROM google_analytics_properties WHERE connection_id = $1",
      values: [connectionId],
    })
    ...
    await this.db.query({
      name: "google-analytics-oauth-delete-states",
      text: "DELETE FROM google_analytics_oauth_states WHERE connection_id = $1",
      values: [connectionId],
    })

    await this.db.query({
      name: "google-analytics-oauth-delete-connection",
      text: "DELETE FROM google_analytics_oauth_connections WHERE id = $1",
      values: [connectionId],
    })
  }
  ...
  async getRawTokenMaterial(connectionId: string) {
    const result = await this.db.query<{
      ...
      name: "google-analytics-oauth-token-material-find",
      text: `
  ...
  async updateTokenMaterial(input: {
    ...
    await this.db.query({
      name: "google-analytics-oauth-token-material-update",
      text: `
        UPDATE google_analytics_oauth_connections
  ...
  async setConnectionLifecycleStatus(input: {
    ...
    await this.db.query({
      name: "google-analytics-oauth-connection-lifecycle-update",
      text: `
        UPDATE google_analytics_oauth_connections
  ...
  async listConnectionIdsByWorkspace(
    ...
    const result = await this.db.query<{ id: string }>({
      name: "google-analytics-oauth-connections-list-by-workspace",
      text: `
        SELECT id
        FROM google_analytics_oauth_connections
  ...
  async listRecentOutboxEvents(connectionId: string, limit: number) {
    const result = await this.db.query<{
      ...
      name: "google-analytics-oauth-outbox-recent-list",
      text: `
        SELECT
          id,
