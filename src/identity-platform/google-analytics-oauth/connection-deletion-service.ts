...
export class GoogleAnalyticsOAuthConnectionDeletionService {
  constructor(private readonly repository: GoogleAnalyticsOAuthRepository) {}
  ...
  async deleteConnection(actor: AuthenticatedActor, connectionId: string) {
    ...
    await this.repository.withTransaction(async () => {
      ...
      await this.repository.saveEvent(
        connectionId,
        "google_analytics.oauth.connection.deleted",
        payload
      )
      await this.repository.appendAuditLog({
        ...
        action: "integration.google_analytics_oauth.deleted",
        entityId: connectionId,
        metadata: payload,
        createdAt: occurredAt,
      })
      await this.repository.appendOutboxEvent({
        eventType: "google_analytics.oauth.connection.deleted",
