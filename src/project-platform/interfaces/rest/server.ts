...
import { ERRORS } from "../../../identity-platform/application/errors/IdentityError"
...
async function authenticate(
  platform: ReturnType<typeof createProjectPlatform>,
  ...
  return platform.identity.commands.resolveActorFromAccessToken(token)
}
...
export function createProjectApiServer(platform = createProjectPlatform()) {
  return createServer(async (request, response) => {
    ...
    try {
      ...
      const actor = await authenticate(platform, request)

      if (method === "POST" && url.pathname === "/v1/projects") {
        const body = createProjectSchema.parse(await readJsonBody(request))
        return send(
          201,
          await projects.createProject(actor, { ...body, organizationId: actor.organizationId })
        )
      }
      ...
      if (projectList) {
        return send(
          200,
          await projects.listProjects(actor, {
      ...
      if (projectMatch && method === "GET") {
        return send(200, await projects.getProject(actor, projectMatch[1]))
      }
      ...
      if (archiveMatch && method === "POST") {
        if (archiveMatch[2] === "archive")
          return send(200, await projects.archiveProject(actor, archiveMatch[1]))
        if (archiveMatch[2] === "restore")
          return send(200, await projects.restoreProject(actor, archiveMatch[1]))
        return send(200, await projects.deleteProject(actor, archiveMatch[1]))
      }
      ...
      if (dataSourceList && method === "POST") {
        return send(
          201,
          await projects.createDataSource(actor, {
      ...
      if (dataSourceList && method === "GET") {
        return send(
          200,
          await projects.listDataSources(actor, {
            projectId: dataSourceList[1],
      ...
      if (dataSourceMatch && method === "POST") {
        ...
        if (action === "enable")
          return send(200, await projects.enableDataSource(actor, dataSourceMatch[1]))
        if (action === "disable")
          return send(200, await projects.disableDataSource(actor, dataSourceMatch[1]))
        if (action === "archive")
          return send(200, await projects.archiveDataSource(actor, dataSourceMatch[1]))
        if (action === "delete")
          return send(200, await projects.deleteDataSource(actor, dataSourceMatch[1]))
      }
      ...
      if (inviteMatch && method === "POST") {
        return send(
          201,
          await projects.inviteProjectMember(actor, {
      ...
      if (memberMatch && method === "POST") {
        ...
        if (memberMatch[3] === "roles")
          return send(
            200,
            await projects.updateProjectMemberRole(actor, {
        ...
        if (memberMatch[3] === "suspend")
          return send(
            200,
            await projects.suspendProjectMember(actor, {
              projectId: memberMatch[1],
              userId: memberMatch[2],
        ...
        return send(
          200,
          await projects.removeProjectMember(actor, {
            projectId: memberMatch[1],
            userId: memberMatch[2],
