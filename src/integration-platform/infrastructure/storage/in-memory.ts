...
class InMemoryConnectorRepository implements ConnectorRepository {
  ...
  async findByConnectorId(connectorId: string) {
    return clone(this.rows.find((row) => row.connectorId === connectorId) ?? null)
  }
  async list() {
    return clone(this.rows)
  }
  async save(connector: ConnectorState) {
    upsertById(this.rows, connector)
  }
}
...
class InMemoryConnectionRepository implements ConnectionRepository {
  ...
  async listByOrganizationId(organizationId: string) {
    return clone(this.rows.filter((row) => row.organizationId === organizationId))
  }
  async listByWorkspaceId(workspaceId: string) {
    return clone(this.rows.filter((row) => row.workspaceId === workspaceId))
  }
  async listByConnectorId(connectorId: string) {
    return clone(this.rows.filter((row) => row.connectorId === connectorId))
  }
  async save(connection: ConnectionState) {
    upsertById(this.rows, connection)
  }
}
...
class InMemoryCredentialRepository implements CredentialRepository {
  ...
  async findLatestByConnectionId(connectionId: string) {
    return clone(this.rows.findLast((row) => row.connectionId === connectionId) ?? null)
  }
  async save(credential: CredentialState) {
    upsertById(this.rows, credential)
  }
}
...
class InMemoryOAuthSessionRepository implements OAuthSessionRepository {
  ...
  async findByState(state: string) {
    return clone(this.rows.find((row) => row.state === state) ?? null)
  }
  async listByConnectionId(connectionId: string) {
    return clone(this.rows.filter((row) => row.connectionId === connectionId))
  }
  async save(session: OAuthSessionState) {
    upsertById(this.rows, session)
  }
}
...
class InMemoryOAuthTokenRepository implements OAuthTokenRepository {
  ...
  async findLatestByCredentialId(credentialId: string) {
    return clone(this.rows.findLast((row) => row.id.startsWith(credentialId)) ?? null)
  }
  async save(token: OAuthTokenState) {
    upsertById(this.rows, token)
  }
}
...
class InMemorySyncJobRepository implements SyncJobRepository {
  ...
  async save(job: SyncJobState) {
    upsertById(this.rows, job)
  }
}
...
class InMemoryWebhookRegistrationRepository implements WebhookRegistrationRepository {
  ...
  async save(registration: WebhookRegistrationState) {
    upsertById(this.rows, registration)
  }
}
...
class InMemoryConnectorHealthRepository implements ConnectorHealthRepository {
  ...
  async findLatestByConnectorId(connectorId: string) {
    return clone(this.rows.findLast((row) => row.connectorId === connectorId) ?? null)
  }
  async save(health: ConnectorHealthState) {
    upsertById(this.rows, health)
  }
}
...
class InMemoryConnectorConfigurationRepository implements ConnectorConfigurationRepository {
  ...
  async save(configuration: ConnectorConfigurationState) {
    upsertById(this.rows, configuration)
  }
}

export function createInMemoryIntegrationRepositories(
  seed?: Partial<Record<keyof IntegrationRepositories, unknown[]>>
): IntegrationRepositories {
  return {
    ...
    oauthSessions: new InMemoryOAuthSessionRepository(
      (seed?.oauthSessions ?? []) as OAuthSessionState[]
    ),
    ...
    webhookRegistrations: new InMemoryWebhookRegistrationRepository(
      (seed?.webhookRegistrations ?? []) as WebhookRegistrationState[]
    ),
    connectorHealth: new InMemoryConnectorHealthRepository(
      (seed?.connectorHealth ?? []) as ConnectorHealthState[]
    ),
    connectorConfigurations: new InMemoryConnectorConfigurationRepository(
      (seed?.connectorConfigurations ?? []) as ConnectorConfigurationState[]
    ),
  }
}
