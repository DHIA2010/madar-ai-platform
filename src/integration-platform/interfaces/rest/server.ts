    ...
    try {
      ...
      if (method === "POST" && url.pathname === "/v1/connectors") {
        return json(
          response,
          201,
          await integrations.registerConnector(
            actorFromRequest(request) as never,
            registerConnectorSchema.parse(await readJsonBody(request))
          )
        )
      }

      const connectorCapabilitiesMatch = url.pathname.match(
      ...
      if (method === "GET" && connectorCapabilitiesMatch) {
        return json(
          response,
          200,
          await integrations.getConnectorCapabilities(
            actorFromRequest(request) as never,
            connectorCapabilitiesMatch[1]
          )
        )
      }
      ...
      if (method === "GET" && connectorHealthMatch) {
        return json(
          response,
          200,
          await integrations.getConnectorHealth(
            actorFromRequest(request) as never,
            connectorHealthMatch[1]
          )
        )
      }
      ...
      if (method === "POST" && url.pathname === "/v1/connections") {
        ...
        return json(
          response,
          201,
          await integrations.createConnection(actorFromRequest(request) as never, {
      ...
      const connectionActionMatch = url.pathname.match(
        /^\/v1\/connections\/([^/]+)\/(disconnect|oauth\/start|sync|webhooks)$/
      )
      if (method === "POST" && connectionActionMatch) {
        ...
        if (action === "disconnect")
          return json(
            response,
            200,
            await integrations.disconnect(actorFromRequest(request) as never, connectionId)
          )
        if (action === "oauth/start")
          return json(
            response,
            200,
            await integrations.startOAuth(actorFromRequest(request) as never, connectionId)
          )
        if (action === "sync") {
          ...
          return json(
            response,
            200,
            await integrations.requestSync(actorFromRequest(request) as never, body)
          )
        }
        if (action === "webhooks") {
          const body = webhookRegistrationSchema.parse({
            ...((await readJsonBody(request)) as Record<string, unknown>),
            connectionId,
          })
          return json(
            response,
            201,
            await integrations.registerWebhook(actorFromRequest(request) as never, body)
          )
        }
      }
      ...
      if (method === "POST" && url.pathname === "/v1/oauth/callback") {
        return json(
          ...
          await integrations.completeOAuth(
            actorFromRequest(request) as never,
            oauthCallbackSchema.parse(await readJsonBody(request))
          )
        )
      }
      ...
      if (method === "POST" && cancelMatch) {
        return json(
          response,
          200,
          await integrations.cancelSync(actorFromRequest(request) as never, cancelMatch[1])
        )
      }
      ...
      if (error instanceof Error && error.name === "IntegrationPlatformError") {
        ...
        return json(response, integrationError.status ?? 400, {
