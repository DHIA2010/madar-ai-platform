...
export default function VisitorsChartCard() {
  const { readModelViewModel } = useWidget("visitors-chart")
  const payload = readModelViewModel?.payload

  const chartData =
    payload?.dataPoints?.map((point) => ({
      month: String(point.month ?? "-"),
      desktop: typeof point.visitors === "number" ? point.visitors : 0,
      sessions: typeof point.sessions === "number" ? point.sessions : 0,
    })) ?? []

  const totalVisitors = chartData.reduce((total, point) => total + point.desktop, 0)
    ...
    <Card className="overflow-hidden h-full">
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        ...
        <div className="text-right">
          ...
          <CardDescription className="text-lg">{payload?.title ?? "إجمالي الزوار"}</CardDescription>
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="mt-0 h-full w-full">
          <BarChart data={chartData} margin={{ left: 0, right: 0 }} barSize={18}>
            {/* ✅ Gradient Definition */}
            <defs>
            ...
            <XAxis hide dataKey="month" axisLine={false} tickLine={false} />

            {/* ✅ Use gradient here */}
            <Bar dataKey="desktop" fill="url(#VisitorbarGradient)" radius={[8, 8, 0, 0]} />
          </BarChart>
        </ChartContainer>
      </CardContent>
    </Card>
